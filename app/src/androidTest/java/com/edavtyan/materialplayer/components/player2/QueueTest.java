...
public class QueueTest extends BaseTest {
	...
	@Test public void addTrack_addTrackToInnerList() {
		Track track = mock(Track.class);
		queue.addTrack(track);
		verify(innerList).add(track);
	}

	@Test public void addManyTracks_addTracksToInnerList() {
		List tracks = mock(List.class);
		queue.addManyTracks(tracks);
		verify(innerList).addAll(tracks);
	}

	@Test public void removeTrackAt_removeTrackFromInnerQueue() {
		queue.removeTrackAt(1);
		verify(innerList).remove(1);
	}

	@Test public void clear_clearInnerList() {
		queue.clear();
		verify(innerList).clear();
	}

	@Test public void getTrackAt_returnTrackFromInnerQueue() {
		Track track = mock(Track.class);
		when(innerList.get(2)).thenReturn(track);
		assertThat(queue.getTrackAt(2)).isSameAs(track);
	}

	@Test public void getTracksCount_returnInnerListSize() {
		when(innerList.size()).thenReturn(3);
		assertThat(queue.getTracksCount()).isEqualTo(3);
	}

	@Test public void moveToNext_notAtLastTrack_incrementPositionAndSetIsEndedToFalse() {
	...
	@Test public void moveToNext_repeatOne_keepPositionAndSetIsEndedToFalse() {
		when(innerList.size()).thenReturn(4);
	...
	@Test public void moveToNext_atLastTrackAndRepeatDisabled_keepPositionAndSetIsEndedToTrue() {
		when(innerList.size()).thenReturn(4);
	...
	@Test public void moveToPrev_inMiddleAndRepeatDisabled_decrementPosition() {
		testMoveToPrevWithRepeatMode(RepeatMode.DISABLED, 2);
	}

	@Test public void moveToPrev_inMiddleAndRepeatAll_decrementPosition() {
		testMoveToPrevWithRepeatMode(RepeatMode.REPEAT_ALL, 2);
	}

	@Test public void moveToPrev_inMiddleAndRepeatOne_keepPositionUnchanged() {
		testMoveToPrevWithRepeatMode(RepeatMode.REPEAT_ONE, 3);
	}

	@Test public void getCurrentTrack_returnTrackAtGivenIndex() {
		Track track = mock(Track.class);
