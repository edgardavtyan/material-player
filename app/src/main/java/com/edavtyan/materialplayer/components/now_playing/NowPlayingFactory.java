...
public class NowPlayingFactory extends BaseFactory {
	...
	public NowPlayingMvp.Presenter providePresenter() {
		if (presenter == null) presenter = new NowPlayingPresenter(provideModel(), provideView());
		return presenter;
	}
	...
	public AppColors provideAppColors() {
		if (appColors == null) appColors = new AppColors(provideActivity());
		return appColors;
	}

	public NowPlayingMvp.Model provideModel() {
		if (model == null) model = new NowPlayingModel(provideActivity());
		return model;
	}

	public NowPlayingMvp.View.Info provideInfo() {
		if (info == null) info = new NowPlayingInfo(provideActivity());
		return info;
	}

	public NowPlayingMvp.View.Art provideArt() {
		if (art == null) art = new NowPlayingArt(provideActivity());
		return art;
	}

	public NowPlayingMvp.View.Seekbar provideSeekbar() {
		if (seekbar == null) seekbar = new NowPlayingSeekbar(provideActivity(), providePresenter());
		return seekbar;
	}

	public NowPlayingMvp.View.Fab provideFab() {
		if (fab == null) fab = new NowPlayingFab(provideActivity(), providePresenter());
		return fab;
	}
}
